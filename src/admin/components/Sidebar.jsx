...
const Sidebar = ({ activeTab, setActiveTab, isSidebarOpen, setIsSidebarOpen, centerName, settings, setSettings }) => {
    ...
    return (
        <aside className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} transition-transform duration-300 ease-in-out lg:flex w-72 bg-[#050b18]/70 lg:bg-transparent glass m-0 lg:m-6 lg:mr-0 flex-col overflow-y-auto fixed lg:relative z-[100] h-screen lg:h-[calc(100vh-3rem)] sidebar-border lg:rounded-[2.5rem] shadow-2xl custom-scrollbar`}>
            ...
            <div className="p-6 space-y-2">
                ...
                >
                    <LogOut size={18} />
                    <span className="text-sm font-medium">시스템 잠금 (로그아웃)</span>
                </button>
            </div>
        </aside>
    );
