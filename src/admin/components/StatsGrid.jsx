                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    >
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h4 className="text-xs font-black text-amber-400 uppercase tracking-widest">만기 예정 회원 (7일)</h4>
                        </div>
                                        ...
                                        <div key={i} className={`p-4 rounded-2xl border flex justify-between items-center transition-all ${isSpecial
                                            ...
                                            <div className="min-w-0 flex-1 mr-3">
                                                <div className="flex items-center gap-2 mb-1">
                                                    ...
                                                    <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded flex-shrink-0 ${isSpecial ? 'text-slate-600 bg-white/5' : 'text-amber-200/70 bg-amber-500/10'}`}>{m.status}</span>
                                                </div>
                    ...
                    >
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h4 className="text-xs font-black text-violet-400 uppercase tracking-widest">맞춤상담 및 원데이</h4>
                        </div>
                        <div className="space-y-3 flex-1 overflow-y-auto max-h-[280px] pr-1 custom-scrollbar">
                            {memberList
                                .filter(m => {
                                    ...
                                    if (!m.end_date) return true; // Show TBD ones too but end_date usually exists
                                    const today = new Date();
                                    today.setHours(0, 0, 0, 0);
                                    const endDate = new Date(m.end_date);
                                    const diff = (endDate - today) / (1000 * 60 * 60 * 24);
                                    return diff >= -1 && diff <= 7; // Show slightly past ones too
                                })
                                .sort((a, b) => new Date(a.end_date) - new Date(b.end_date))
                                .slice(0, 5)
                                .map((m, i) => (
                    ...
                    >
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h4 className="text-xs font-black text-emerald-400 uppercase tracking-widest">신규 등록 회원 (7일)</h4>
                        </div>
                        <div className="space-y-3 flex-1 overflow-y-auto max-h-[280px] pr-1 custom-scrollbar">
                            {memberList
                                .filter(m => {
